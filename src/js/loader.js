...
import loadingMoon from '../img/loadingMoon.png'
import loadingMoonBox from '../img/loadingMoonBox.png'
...
let loaderbarLogo = new Sprite(loadingMoon);
let loaderbarLogogray = new Sprite(loadingMoonBox);
let loaderBkg = new Sprite(bkg);
loaderBkg.attr({
  anchor:[0.5,0.5],
...
loadinglayer.append(loaderBkg,loaderbarLogogray); //loading页面加入进度条
  ...
  async function loadRes() {        //异步加载图片资源方法
    stage.on('preload', (evt) => {  //异步加载监听
      if (evt.loaded.length/evt.resources.length == 1) {
        ...
        loaderBarlight.attr({
            size: [325 * evt.loaded.length/evt.resources.length, 331],
        })
      }
    })
