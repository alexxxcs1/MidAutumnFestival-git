...
const tool = {
    ...
    getShare(url){
        let shareiconurl = shareicon.split('/');
        let share_url = location.href;
        let share_img = 'http://client.rup-china.com/midautumnfestival/' + shareiconurl[1]+'/'+shareiconurl[2];
        let share_title = '嫦娥“助”你中秋无“幽”';
        let share_content = '';
        api.getShare(share_url).then((response) => {
    ...
    GetUrlParam(paraName) {
        var url = document.location.toString(),
            arrObj = url.split("?");
        if (arrObj.length > 1) {
            for (var arr, arrPara = arrObj[1].split("&"), i = 0; i < arrPara.length; i++)
                if (arr = arrPara[i].split("="), null != arr && arr[0] == paraName) return arr[1];
            return ""
        }
        return ""
    }
